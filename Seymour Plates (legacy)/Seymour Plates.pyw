...
class StartQT4(QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.all_btn.clicked.connect(self.all)
        self.ui.none_btn.clicked.connect(self.none)
    ...
    def all(self):
        all_none('all', mywindow)
        
        
    def none(self):
        all_none('none', mywindow)
...
def all_none(type, window):
    #Function for the all and none buttons. Sets all 'selected' attribute of plate instances to True or False
    for plate in plates:
        if type == 'all':
            plate.selected = True
        elif type == 'none':
