...
DIR_RES = os.path.abspath(ospj(module_locator.path(), os.pardir, 'resources'))
...
class Main(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        # These would ideally be done in Designer, but it's not supported.
        self.ui.tree_display.setColumnWidth(0, 75) 
        self.ui.tree_display.setColumnWidth(1, 150) #The last column's width is automatic.
    ...
    def save(self):
        ...
        saved = {'plates': _plates, 'combined': combined}
        save(saved, ospj(DIR_RES, 'save.yaml'))
...
def combine(temp_path, save_path):
    """Merge PDFs in the temp directory into Plates.pdf, also in the temp directory."""
...
def save(dataset, filepath):
    """Save the plates list or combined option to a YAML file"""
    try:
        with open(filepath, 'w') as f:
            f.write(yaml.dump(dataset))
            main.ui.statusbar.showMessage("Selections saved")
    except PermissionError as e:
        main.ui.statusbar.showMessage("Error saving:", filepath, "is open. Close and try again.")
        
def load(filename):
    """Return the data from a yaml file.  Used for loading saved data."""
    if exists(filename):
        with open(filename, 'r') as f:
            return yaml.load(f)
    else:
        return {'plates': [], 'combined': True}
            
def load_idents(filepath):
    try:
...
if __name__ == '__main__':
    ...
    loaded = load(ospj(DIR_RES, 'save.yaml'))
    _plates = loaded['plates']
    combined = loaded['combined']
    idents = load_idents(ospj(DIR_RES, 'idents.json'))
